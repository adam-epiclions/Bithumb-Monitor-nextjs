...
class BithumbAPI:
    ...
    def __init__(self, access_key=None, secret_key=None, account_number=None):
        
        self.base_url = "https://api.bithumb.com"
        
        # 계정 번호가 있는 경우 해당 계정의 API 키를 사용
        if account_number:
            self.access_key = access_key or os.getenv(f'BITHUMB_ACCESS_KEY_{account_number}')
            self.secret_key = secret_key or os.getenv(f'BITHUMB_SECRET_KEY_{account_number}')
        else:
            # 기본 API 키 사용
    ...
    def get_all_prices(self) -> dict:
        """
        전체 마켓(KRW 기준)의 현재가를 한 번에 가져오는 메서드
        Returns:
            dict: {코인심볼(str): 현재가(float)} 형태의 딕셔너리
        """
        ...
        try:
            ...
            for symbol, info in data.items():
                if isinstance(info, dict) and "closing_price" in info:
                    price_map[symbol] = float(info["closing_price"])
            return price_map
    ...
    def _create_jwt_token(self, query_params=None):
        ...
        if query_params:
            ...
            hash_obj.update(encoded_params.encode())
